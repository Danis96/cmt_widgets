...
class AW_SwitchWithTitleDescriptionInputField extends StatelessWidget {
  const AW_SwitchWithTitleDescriptionInputField({
    Key? key,
  ...
  Widget _textAndInfo() {
    return Row(
      children: <Widget>[
        ...
        IconButton(key: const Key('switch_w_title_desc_field_icon'), onPressed: () => onIconPressed != null ? onIconPressed!() : null, icon: icon),
      ],
    );
  }
  ...
  Widget _textAndSwitch() {
    return Row(
      children: <Widget>[
        ...
        CupertinoSwitch( key: const Key('switch_w_title_desc_field_switch'),
            value: switchBool, onChanged: (bool value) => onChanged(value), activeColor: switchActiveColor),
      ],
    );
  }
